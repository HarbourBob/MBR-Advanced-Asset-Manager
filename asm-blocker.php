...
add_action('wp_head', function () {
    ...
    (function() {
        ...
        console.log('[ASM] Preview mode:', isPreview, '| Post ID:', postID);

        function readPreviewData(){
            try{
                const stored = localStorage.getItem('mbr_asm_preview_blocklist');
                if (!stored) return null;
                
                const expires = parseInt(localStorage.getItem('mbr_asm_preview_expires'), 10);
                if (expires && Date.now() > expires) {
                    localStorage.removeItem('mbr_asm_preview_blocklist');
                    localStorage.removeItem('mbr_asm_preview_expires');
                    return null;
                }
        ...
        if (isPreview) {
            console.log('[ASM] In preview mode, reading localStorage...');
            const preview = readPreviewData();
            console.log('[ASM] Preview data:', preview ? preview.length + ' items' : 'null');
            if (Array.isArray(preview)) {
                ...
                console.log('[ASM] Preview blocking:', styles.length, 'styles and', scripts.length, 'scripts');
            }
        } else {
            console.log('[ASM] NOT in preview mode, using saved blocklist');
        }
